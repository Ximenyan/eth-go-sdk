...
import (
	...
	"github.com/ximenyan/eth-go-sdk/erc20"
...
func (client *Client) call(method string, result interface{}, args ...interface{}) error {
	...
	if err != nil {
		fmt.Println("response.GetObject:",response.Result)
		return err
	}
	...
	//client.Debug(buff.String())
	e := response.GetObject(result)
	if e != nil{
		js, _ := json.Marshal(response.Result)
		fmt.Println("response.GetObject:",js)
	}
	return e
}
